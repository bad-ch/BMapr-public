...
namespace BMapr.GDAL.WebApi.Services
{
    public class MapserverService
    {
        ...
        public MapserverService(IMemoryCache cache, Config config, string project)
        {
            ...
            var mapContent = MapFileService.GetMapContentFromCache(cache, mapMetadata.Value.Key, mapMetadata.Value.FilePath, config, project) ?? string.Empty;

            Ini();

            // todo murks because first method don't allow relative path
            try
            {
                Map = new mapObj(mapContent, 1);
                return;
            ...
            try
            {
                Map = new mapObj(mapMetadata.Value.FilePath);
            }
            catch
            {
