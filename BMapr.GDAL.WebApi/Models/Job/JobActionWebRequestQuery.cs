...
namespace BMapr.GDAL.WebApi.Models.Job
{
    public class JobActionWebRequestQuery : JobActionWebRequest
    {
        ...
        public override Result Execute()
        {
            ...
            HtmlNodeCollection queryNodes;

            if (!string.IsNullOrEmpty(ByAttribute))
            {
                queryNodes = htmlDocument.DocumentNode.SelectNodes($"//descendant::*[contains(@{ByAttribute}, '{ByAttributeValue}')]");
            }
            else
            {
                queryNodes  = htmlDocument.DocumentNode.SelectNodes(XPath);
            }
