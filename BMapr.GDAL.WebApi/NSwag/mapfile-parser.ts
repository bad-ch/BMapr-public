...
import * as Defs from './mapfile-definitions';
...
export class MapfileParser {
  ...
  public static parse(text: string, baseDir?: string): Defs.MapObj {
    ...
    const stack: Context[] = [];
    stack.push({ type: MapfileParser.CtxType.Map, node: map });
    ...
    for (const raw of lines) {
      ...
      if (head === 'FEATURE') {
        ...
        if (ctxFeat.type !== MapfileParser.CtxType.Layer) {
      ...
      if (head === 'POINTS') {
        ...
        if (ctxSym.type !== MapfileParser.CtxType.Symbol) {
      ...
      // Open blocks
      if (head === 'MAP') {
        if (ctx.type !== MapfileParser.CtxType.Map || !(ctx.node instanceof Object)) {
      ...
      if (head === 'WEB') {
        ...
        stack.push({ type: MapfileParser.CtxType.Web, node: web });
        continue;
      }

      if (head === 'METADATA') {
        switch (ctx.type) {
          case MapfileParser.CtxType.Web:
            stack.push({ type: MapfileParser.CtxType.Metadata, node: (ctx.node as Defs.WebObj).metadata });
            break;
          case MapfileParser.CtxType.Layer:
            stack.push({ type: MapfileParser.CtxType.Metadata, node: (ctx.node as Defs.LayerObj).metadata });
            break;
          case MapfileParser.CtxType.Class:
            stack.push({ type: MapfileParser.CtxType.Metadata, node: (ctx.node as Defs.ClassObj).metadata });
            break;
          default:
      ...
      if (head === 'VALIDATION') {
        switch (ctx.type) {
          case MapfileParser.CtxType.Layer:
            stack.push({ type: MapfileParser.CtxType.Validation, node: (ctx.node as Defs.LayerObj).validation });
            break;
          case MapfileParser.CtxType.Class:
            stack.push({ type: MapfileParser.CtxType.Validation, node: (ctx.node as Defs.ClassObj).validation });
            break;
          default:
      ...
      if (head === 'PROJECTION') {
        if (ctx.type === MapfileParser.CtxType.Map) {
          stack.push({ type: MapfileParser.CtxType.Projection, node: (ctx.node as Defs.MapObj).projection });
          continue;
        }
        if (ctx.type === MapfileParser.CtxType.Layer) {
          stack.push({ type: MapfileParser.CtxType.Projection, node: (ctx.node as Defs.LayerObj).projection });
          continue;
        }
      ...
      if (head === 'OUTPUTFORMAT') {
        ...
        stack.push({ type: MapfileParser.CtxType.OutputFormat, node: of });
        continue;
      }
      ...
      if (head === 'SYMBOL') {
        if (ctx.type === MapfileParser.CtxType.Map) {
          ...
          stack.push({ type: MapfileParser.CtxType.Symbol, node: sym });
          continue;
        }
        if (ctx.type === MapfileParser.CtxType.Layer) {
          const sym: Defs.SymbolObj = { attributes: {} };
      ...
      if (head === 'LAYER') {
        ...
        stack.push({ type: MapfileParser.CtxType.Layer, node: layer });
        continue;
      }
      ...
      if (head === 'CLASS') {
        ...
        stack.push({ type: MapfileParser.CtxType.Class, node: c });
        continue;
      }

      if (head === 'STYLE') {
        if (ctx.type === MapfileParser.CtxType.Class) {
          const s: Defs.StyleObj = { pattern: [], attributes: {} };
          (ctx.node as Defs.ClassObj).styles.push(s);
          stack.push({ type: MapfileParser.CtxType.Style, node: s });
          continue;
        }
        if (ctx.type === MapfileParser.CtxType.Label) {
          const s: Defs.StyleObj = { pattern: [], attributes: {} };
          (ctx.node as Defs.LabelObj).styles.push(s);
          stack.push({ type: MapfileParser.CtxType.Style, node: s });
          continue;
        }
        if (ctx.type === MapfileParser.CtxType.Leader) {
          const s: Defs.StyleObj = { pattern: [], attributes: {} };
      ...
      if (head === 'LABEL') {
        ...
        stack.push({ type: MapfileParser.CtxType.Label, node: l });
        continue;
      }
      ...
      if (head === 'LEADER') {
        ...
        stack.push({ type: MapfileParser.CtxType.Leader, node: ld });
        continue;
      }
      ...
      if (head === 'JOIN') {
        ...
        stack.push({ type: MapfileParser.CtxType.Join, node: j });
        continue;
      }
      ...
      if (head === 'IDENTIFY') {
        ...
        stack.push({ type: MapfileParser.CtxType.Identify, node: (ctx.node as Defs.LayerObj).identify });
        continue;
      }
