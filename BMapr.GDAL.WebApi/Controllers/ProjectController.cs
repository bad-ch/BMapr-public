...
namespace BMapr.GDAL.WebApi.Controllers
{
    ...
    public class ProjectController : DefaultController
    {
        ...
        public ActionResult GetLegend(string project, [FromQuery(Name = "token")] string? token)
        {
            ...
            var mapContent = MapFileService.GetMapContentFromCache(_cache, mapMetadata.Value.Key, mapMetadata.Value.FilePath, Config, project) ?? string.Empty;
            var mapServer = new Mapserver(mapContent);
